...
class Glow(Method[GlowInput, PreprocGlowInput, GlowOutput, Tensor]):
    ...
    def generate(
        self, input_file_name: str, out_file_dir: str
    ) -> List[Tuple[GlowInput, GlowOutput]]:
        dataset = list(
            datagen.generate_glow_dataset(
                input_file_name, out_file_dir, self.datagen_options
            )
        )
        return dataset
    ...
    def filter_ill_formed(self, list_samples):
        ...
        for sample in list_samples:
            ...
            new_types = [
                v
                for (i, v) in enumerate(glow_output.types)
                if len(glow_input.vars[i].nodes) > 0
            ]
    ...
    def preproc_output(
        self, input: GlowInput, output: GlowOutput, fmt: str = "onehot"
    ) -> Tensor:
    ...
    def preproc_fine_tuning(
        self, dataset: List[Tuple[GlowInput, GlowOutput]]
    ) -> Tuple[List[Tuple[GlowInput, GlowOutput]], List[Tuple[GlowInput, GlowOutput]]]:
        train_set, test_set = [], []
        ...
        for i, o in dataset:
            if len(i.vars) > self.args.type_hint_count:
                ...
                train_vars, train_types = (
                    [vs for (vs, _) in train_var_types],
                    [ts for (_, ts) in train_var_types],
                )
                train_input = GlowInput(
                    i.input_file_name,
                    i.directory,
                    i.file_name,
                    i.function_name,
                ...
                test_vars, test_types = (
                    [vs for (vs, _) in test_var_types],
                    [ts for (_, ts) in test_var_types],
                )
                test_input = GlowInput(
                    i.input_file_name,
                    i.directory,
                    i.file_name,
                    i.function_name,
            ...
            for i in range(n):
                ...
                for low_pc, _, dwloc in locs:
                    if isinstance(dwloc, CfaLocation):
                        predict.add_to_set_valued(
                            btype_dict, ("cfa", dwloc.arg), ("pc", low_pc, btype)
                        )

                    elif isinstance(dwloc, RegLocation):
                        predict.add_to_set_valued(
                            btype_dict, ("reg", dwloc.reg_num), ("pc", low_pc, btype)
                        )

                    elif isinstance(dwloc, AddrLocation):
                        predict.add_to_set_valued(
                            btype_dict, ("addr", dwloc.arg), ("pc", low_pc, btype)
                        )
